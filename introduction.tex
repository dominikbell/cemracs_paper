...
\end{equation}
with the variables $\bx \in \Omega \subseteq \real^3$, the position of the gyro-centre, $\vp \in \real^3$, the velocity parallel to the magnetic field lines, and $\mu$, the modified magnetic moment. $H$ will be discussed below.
...
\begin{subequations}
	...
	\end{align}
\end{subequations}
Both fast and slow steps are divergence-free and derived from Poisson bracket.
...
with the bracket in polar coordinates such that
\[ \{\phi, f \} = \frac{1}{rB_0}\partial_r \phi\partial_\theta f -\frac{1}{rB_0}\partial_\theta \phi\partial_r f, \qquad \nabla_\parallel = \bb \cdot \nabla. \]
...
This is Poisson equation and in the code it is solved using Finite element method (FEM) on a b-spline basis.

Boundary conditions on $f$:\\
    \begin{itemize}
        \item Periodicity along $\theta$, $z$ and $v_\parallel$.
...
Boundary conditions on $\phi$ :\\
    \begin{itemize}
        \item Periodic along $\theta$ and $v_\parallel$.
        \item Decomposing $\phi$ into Fourier modes, at $r_\text{min}$, we take homogeneous Neumann boundary conditions for the zeroth mode and homogeneous boundary conditions for the others. At $r_\text{max}$, we just take homogeneous Dirichlet boundary conditions.
\end{itemize}

\textbf{Conserved Quantities}\\
Firstly, we have conservative form \eqref{conservation1}. Secondly, \eqref{drift-kinetic model} conserves arbitrary functions
of $f$  along nonlinear characteristics,
\begin{equation}
\frac{D}{Dt} C(f)+\bracket{C(f)}{H} =0,
\end{equation}
and the Casimir invariant $\int C( f ) d^5R$ is also conserved.
Therefore, in addition to the particle number or the L1 norm $\int f d^5R$, the system has an infinite number of conserved quantities such as the L2 norm $\int f^2 d^5R$, and the kinetic entropy $S=\int f \ln(f) d^5R$. 
...
\begin{subequations}
	...
	\end{align}
\end{subequations}
where $E_k$ is the kinetic energy and $E_f$ is the field energy, please see the details in \cite{idomura2008conservative}.