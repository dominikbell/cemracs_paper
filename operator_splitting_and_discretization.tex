...
The Arakawa scheme provably preserves the following quantities: %TODO: add proof (or to reffer to the paper and add just in polar coordinates)?
	...
	\end{align*}
First we will focus on discretization of Poisson bracket in Cartesian coordinates. The scheme in polar coordinates will be discussed later.
...
In polar coordinates, the Arakawa scheme has the same conservation properties. In the paper \cite{Arakawa_1966}, we can find the scheme in curvilinear coordinates. For our purposes, since we want to discretize the Poisson bracket \eqref{eq:poisson_bracket} in the $(r,\theta)$ plane, our desired quantities under the change of coordinates are
\begin{align*}
...
\begin{equation*}
\int \{{\phi,f}\}r\d r \d \theta=\int\frac{1}{r}(-\partial_\theta\phi\partial_r f+\partial_r\phi\partial_\theta f)r\d r \d=0,
\end{equation*}
where the Poisson bracket is discretized as in the Cartesian coordinates. For the other two quantities, the reasoning is analogous.